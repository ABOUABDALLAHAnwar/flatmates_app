...
class PdfReport:
    ...
    def generate(self, flatemate1 : FlateMate, flatemate2 : FlateMate, the_bill : Bill):
        ...
        pdf.cell(w=100, h=40, txt="Periode ", border=0)
        pdf.cell(w=300, h=40, txt=the_bill.periode , border=0, ln=1)
        pdf.set_font(family='Times', size=14, style='B')
        pdf.cell(w=100, h=20, txt=flatemate1.name, border=0)
        pdf.cell(w=100, h=20, txt=str(round(flatemate1_pays, 2)), border=0, ln=1)
        pdf.cell(w=100, h=20, txt=flatemate2.name, border=0)
        pdf.cell(w=100, h=20, txt=str(round(flatemate2_pays, 2)), border=0, ln=1)
